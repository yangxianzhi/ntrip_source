...
%%%
%%% @end
%%% Created : 12. 七月 2017 17:01
%%%-------------------------------------------------------------------
-module(ntrip_source_mnesia).
-author("ilink").
...
create_tables() ->
  ntrip_source_boot:apply_module_attributes(boot_mnesia).

%% @doc Copy mnesia tables.
copy_tables() ->
  ntrip_source_boot:apply_module_attributes(copy_mnesia).
...
cluster_status() ->
  ...
  case (Stopped =:= []) of
    ...
    false-> [{running_nodes, Running}, {stopped_nodes, Stopped}]
  end.
